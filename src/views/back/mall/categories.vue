...
export default {
  ...
  methods: {
    ...
    getIcon(iconId) {
      return getIconById(iconId, 'product')
    },
    ...
    async saveCategory() {
      ...
      try {
        if (this.editingId) {
          await updateProductCategory(this.editingId, this.form)
          this.$message.success('分类更新成功')
        } else {
          await createProductCategory(this.form)
  ...
  async mounted() {
    this.availableIcons = getAllIcons('product')
    await this.loadCategories()
  }
}
</script>
